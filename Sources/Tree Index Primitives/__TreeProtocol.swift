...
// its own consumer-facing protocol `Traversable` (`extension __Tree: __TreeProtocol
// where S: __TreeStorage`). The STORAGE column `S` never conforms it — it conforms the
// SEPARATE storage capability ``__TreeStorage``; only the carrier conforms this consumer
// protocol.
//
...
//
// `Tree` is a [DS-028] generic front-door typealias (`Tree.FrontDoor.swift`), not a
// namespace: member-type lookup does not look through an unbound generic typealias on
// any toolchain (swift-institute/Issues#81), so there is no `Tree.Protocol` spelling to
// nest into. The capability protocol therefore stays top-level and non-underscored, as
// `Iterable`, `Buildable`, and `Initiable` already do. `__TreeProtocol` remains as a
// compatibility alias for this protocol.

/// The tree consumer abstraction.
///
