...
extension Tree.N.Inline where n == 2, Element: Copyable {
    /// Constructs a fixed-capacity inline binary tree from a result-builder closure.
    ///
    /// Wraps the dynamic `Tree<E>.N<2>.Builder` per Round-2 Option Y.
    /// Elements placed in BFS level-order. Overflow throws.
    @_disfavoredOverload
    public init(
        @Tree<Element>.N<2>.Builder _ builder: () -> [Element]
    ) throws(Tree.N.Inline.Error) {
        self.init()
